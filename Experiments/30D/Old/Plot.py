...
#%% get the 6 minima from AllLocalMinimaDivergent with the lowest values

AllLocalMinimaDivergentValue=np.sort(AllLocalMinimaDivergentValue)


AllLocalMinimaDivergentValue=AllLocalMinimaDivergentValue[0:6]

# get the corresponding minima

MinimaDivergent=[]
for i in range(6):
    MinimaDivergent.append(AllLocalMinimaDivergent[np.where(AllLocalMinimaDivergentValue[i]==AllLocalMinimaDivergentValue)[0][0]])

MinimaDivergentCG=[]
# run CG from this minima
for i in range(6):
    res = minimize(fct_Rastrigin, MinimaDivergent[i], method='CG',options={'disp': False, 'maxiter':1000000})
    MinimaDivergentCG.append(res.x)

# calculate the euclidean distance between the MinimaDivergentCG

DistanceDivergent=np.zeros((6,6))
for i in range(6):
    for j in range(6):
        DistanceDivergent[i,j]=np.linalg.norm(MinimaDivergentCG[i]-MinimaDivergentCG[j])
