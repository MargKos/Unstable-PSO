...
def fct_average(Parameters, eps): # Paramerter: Damped, Overdamped, Divergent eps=radius of ball
    ...
    for s in range(sim):
        print(np.shape(np.load(path+'AllScores2'+str(name)+str(eps)+str(s)+'.npy')*120))
        Average=Average+np.load(path+'AllScores2'+str(name)+str(eps)+str(s)+'.npy')*120 # multiplied by 120, if one wants the toal number instead of percentage
